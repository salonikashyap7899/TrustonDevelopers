...
export function SobhaStyleNav() {
  ...
  const topLinks = allLinks?.filter((l) => !l.parent_id) || [];
  const servicesLink = topLinks.find((l) => l.label.toUpperCase() === "SERVICES");
  const otherTopLinks = topLinks.filter((l) => l.label.toUpperCase() !== "SERVICES");

  const midPoint = Math.ceil(otherTopLinks.length / 2);
  const leftLinks = otherTopLinks.slice(0, midPoint);
  const rightTopLinks = otherTopLinks.slice(midPoint);

  const subServices = allLinks?.filter((l) => l.parent_id === servicesLink?.id) || [];
